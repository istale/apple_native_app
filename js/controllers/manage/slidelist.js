...
app.controller('ManageSlideListCtrl', ['$scope', '$ionicLoading', '$ionicViewService', '$http', '$rootScope', 'UrlHelper', '$stateParams', function ($scope, $ionicLoading, $ionicViewService, $http, $rootScope, UrlHelper, $stateParams) {
    ...
    $scope.doRefresh = function () {
        ...
        var url = UrlHelper.prepareApiUrl("webapiThumbnailImages/" + $stateParams.event_url);
        var thumbnail_url = UrlHelper.prepareDataUrl("");
        ...
        .success(function (data, status, headers, config) {
            ...
            for (var i = 0; i < data.length; i++) {
                var isSlideExist = 0;
                for (var j = 0; j < $rootScope.slides.length; j++) {
                    if (data[i].id == $rootScope.slides[j].image_id) {
                        console.log('data.id:', data[i].id);
                        console.log('slides.image_id:', $rootScope.slides[j].image_id);
                        isSlideExist = 1;
                    }

                    if ((j == $rootScope.slides.length - 1) && (isSlideExist == 0)) {

                        console.log('isSlideExist: ', isSlideExist);

                        var newSlideIndex = $rootScope.slides.length + 1;
                        $rootScope.slides.push({
    ...
    $scope.loadMore = function () {
        if ($scope.imageShown + 10 >= $rootScope.slides.length) {
            //$scope.$apply(function () {
            $scope.imageShown = $rootScope.slides.length;
            //});
    ...
    $scope.moreDataCanBeLoaded = function () {
        if ($scope.imageShown == $rootScope.slides.length) {
            return false;
        }
        return true;
    };
    ...
    $scope.slideIsPublishChange = function (slide_id) {
        ...
        var slide = $rootScope.slides[slide_id - 1];
        ...
        var url = UrlHelper.prepareApiUrl("webapiImages/" + $stateParams.event_url + "/" + slide.image_id);
